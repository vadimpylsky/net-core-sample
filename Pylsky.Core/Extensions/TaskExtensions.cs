...
public static class TaskExtensions
{
    ...
    /// </returns>
    /// <example>
    ///     If you need timeout with cancellation, use the better approach with
    ...
    /// <summary>
    ///     Returns a task that completes as the original task completes or when a timeout expires, whichever happens first.
    ///     For logging exceptions of <paramref name="task" /> use <see cref="WithLoggingErrors{T}" /> method before calling
    ///     this.
    /// </summary>
    ...
    public static Task WithLoggingErrors(this Task task, IPylskyLogger logger)
    {
        task.ContinueWith(
    ...
    /// </returns>
    /// <exception cref="T:System.ArgumentNullException">When <paramref name="logger" /> is null.</exception>
    public static Task<T> WithLoggingErrors<T>(this Task<T> task, IPylskyLogger logger)
    {
    ...
    /// <param name="task">The task.</param>
    /// <param name="logger">Logger implementation.</param>
    public static void FireAndForget(this Task task, IPylskyLogger logger)
    {
...
#pragma warning restore RECS0165 // Asynchronous methods should return a Task instead of void
    ...
    private static void LogException(Exception exception, IPylskyLogger logger)
    {
